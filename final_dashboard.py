...
if menu == "Dashboard":
    ...
    with tab1:
        ...
        st.info("""
        This dashboard presents an exploratory analysis of the Wine Quality dataset, focusing on understanding chemical features, visualizing patterns, and identifying key factors that influence wine quality to support machine learning insights.
        """)
        ...
        # sns.heatmap(df.corr(numeric_only=True), annot=True, cmap="RdBu_r", ax=ax)
        # st.pyplot(fig)
        ...
        st.success("""
        This dashboard explores how chemical properties such as alcohol, acidity, and sulphates influence wine quality. 
        It helps identify patterns, correlations, and key factors that play a major role in determining wine quality.
        """)
   ...
   # ================= UNIVARIATE TAB =================
    with tab4:
        ...
        st.write("### Correlation Heatmap of wine Features")
        fig, ax = plt.subplots(figsize=(12,7))
        sns.heatmap(df.corr(numeric_only=True), annot=True, cmap="RdBu_r", ax=ax)
        st.pyplot(fig)
        st.write("Key Insights:")
        st.write("- Alcohol has a positive correlation with quality (higher alcohol → better quality).")
        st.write("- Volatile acidity has a negative correlation with quality (higher acidity → lower quality).")
        st.write("- Density is negatively related to alcohol.")
        st.write("- Sulphates show a moderate positive effect on quality.")
